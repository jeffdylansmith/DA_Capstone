...
where date_trunc('day', w.dt_iso)::date not in (
	...
	FROM weather w
	LEFT JOIN accidents a on date_trunc('day', a.dateandtime)::date = date_trunc('day', w.dt_iso)::date --and (a.weatherdescription = 'RAIN' or a.weatherdescription = 'SNOW')
	LEFT JOIN accidents f on date_trunc('day', f.dateandtime)::date = date_trunc('day', w.dt_iso)::date  and f.numberofinjuries > 0
	where date_part('year', w.dt_iso) > 2012
	--w.snow_1h is not null and weather_main = 'Snow'
	AND w.rain_1h is not null and w.snow_1h is not null and (weather_main = 'Thunderstorm' or weather_main = 'Rain' or weather_main = 'Snow')
	group by w.dt_iso
	)
